...
""" gmsh_box.py     => This script generates meshes for a 3D cuboid structure (cube or rectangular prism) using gsmh method desribed in the refrences. An application of this script can be found in [3].
...
# faces of cube: curve1 and curve4 are disabled due to being in shadow regarding the incident plane wave
#curve1 = gmsh.model.geo.add_curve_loop([line1, line2, line3, line4])     ## or curve1 = gmsh.model.geo.addCurveLoop([line1, line2, line3, line4])
curve2 = gmsh.model.geo.add_curve_loop([line5, line6, line7, line8])
curve3 = gmsh.model.geo.add_curve_loop([line9, line5, line10, -line4])
#curve4 = gmsh.model.geo.add_curve_loop([line9, -line8, -line12, line3])
curve5 = gmsh.model.geo.add_curve_loop([line6, line11, -line1, -line10])
curve6 = gmsh.model.geo.add_curve_loop([line11, line2, line12, -line7])


# Before constructing the surface, we must define the curves of holes (which need points and lines of holes) that must be subtracted.
if aperture == True:
	point1_hole = gmsh.model.geo.add_point(-l_app/2, -w_app/2, -h/2, lc)                            
	point2_hole = gmsh.model.geo.add_point(l_app/2, -w_app/2, -h/2, lc)
	point3_hole = gmsh.model.geo.add_point(l_app/2, w_app/2, -h/2, lc)
	point4_hole = gmsh.model.geo.add_point(-l_app/2, w_app/2, -h/2, lc)
	line1_hole = gmsh.model.geo.add_line(point1_hole, point2_hole)				
	line2_hole = gmsh.model.geo.add_line(point2_hole, point3_hole)
	line3_hole = gmsh.model.geo.add_line(point3_hole, point4_hole)
	line4_hole = gmsh.model.geo.add_line(point4_hole, point1_hole)
	curve1_hole = gmsh.model.geo.add_curve_loop([line1_hole, line2_hole, line3_hole, line4_hole])


# surfaces of cube: surface1 and surface4 are disabled since it is in the shadow region

#if aperture == True:
	#surface1 = gmsh.model.geo.add_plane_surface([curve1, curve1_hole])     As a general rule, if a surface has N holes, it is defined by N+1 curve loops: the first loop defines the exterior boundary; the other loops define the boundaries of the holes.
#else:
	#surface1 = gmsh.model.geo.add_plane_surface([curve1])
	
surface2 = gmsh.model.geo.add_plane_surface([curve2])
surface3 = gmsh.model.geo.add_plane_surface([curve3])
#surface4 = gmsh.model.geo.add_plane_surface([curve4])
surface5 = gmsh.model.geo.add_plane_surface([curve5])
surface6 = gmsh.model.geo.add_plane_surface([curve6])
